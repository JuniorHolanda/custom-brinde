...
export function showMockup(
  ...
  for (let i = 0; i < listMockupPart.length; i++) {
    ...
    if (listMockupPart[i] !== "") {
      ...
      if (
        listMockupPart[i] !== ajuste &&
        controlForProducts.childElementCount == 0
      ) {
        ...
        btnPartProduct.addEventListener("click", () =>
          createBtnColor(fileName)
        ); //adiciona um ouvinte ao botão que chama a função que cria os botões de cores com o argumento o nome da parte
      } // fecha o if
    } // fecha o if
  } // fecha o for
